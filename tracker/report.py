...
def _append_premium_section(lines, periods=None, end_date=None):
    ...
    lines += [
        ...
        "> | 加仓阈值 | 每天算一个值：最低溢价 + (最高溢价 - 最低溢价)×25%，然后取窗口内所有天的平均。最低溢价=最低价/前一日净值-1，最高溢价=最高价/前一日净值-1。溢价低于此值时加大定投 |",
        "> | 减仓阈值 | 每天算一个值：最低溢价 + (最高溢价 - 最低溢价)×75%，然后取窗口内所有天的平均。溢价高于此值时减少定投 |",
        "> | 净值涨幅 | 末日净值 / 首日净值 - 1 |",
        "> | 场内涨幅 | 末日收盘价 / 首日收盘价 - 1 |",
...
def _append_method(lines, idx_label, idx_note):
    ...
    lines += [
        ...
        "| **加仓阈值** | 每天算一个值：最低溢价 + (最高溢价 − 最低溢价) × 25%，然后取窗口内所有天的平均。最低溢价 = 最低价/前一日净值−1，最高溢价 = 最高价/前一日净值−1。溢价 < 此值 → 加大定投 |",
        "| **减仓阈值** | 每天算一个值：最低溢价 + (最高溢价 − 最低溢价) × 75%，然后取窗口内所有天的平均。溢价 > 此值 → 减少定投 |",
